...
echo "Querying for default GKE TEST cluster"
PROJECT="$1"
OLD_IFS="$IFS"
IFS=$'\n'
clusters=($(gcloud --project "$PROJECT" container clusters list --filter='resourceLabels.tapad-gke-environment=current AND resourceLabels.tapad-gke-cluster=test' | grep RUNNING))
IFS="$OLD_IFS"
